import type { TipTapDoc } from "@/lib/documents";
...
export interface DocumentTemplate {
  ...
  title: string;
  tagline: string;
  /** Hex accent used for the gallery badge and thumbnail title bar. */
  accent: string;
  /** Built-in banner art under /public, shown in the doc and its thumbnail. */
  banner: string | null;
...
function bulletList(items: (string | Node[])[]): Node {
  return {
    type: "bulletList",
    content: items.map((item) => ({
      type: "listItem",
      content: [paragraph(item as string | Node[])],
    })),
  };
}
...
function taskList(items: { label: string; checked?: boolean }[]): Node {
  return {
    type: "taskList",
    content: items.map((item) => ({
      type: "taskItem",
...
function image(src: string, alt: string): Node {
  return { type: "image", attrs: { src, alt } };
}

function quote(value: string): Node {
  return { type: "blockquote", content: [paragraph(value)] };
}

function doc(...content: Node[]): TipTapDoc {
