...
function Line({ className, dark }: { className?: string; dark?: boolean }) {
  return (
    <span
      aria-hidden="true"
      className={cn(
      ...
      >
        {Array.from({ length: 6 }).map((_, i) => (
          <span
            ...
            className={cn(
              "h-2.5",
              i < 3 ? "bg-foreground/25" : "bg-foreground/10",
            )}
          />
        ))}
      </span>
...
function PreviewBody({ template }: { template: DocumentTemplate }) {
...
export function TemplateThumbnail({ template }: { template: DocumentTemplate }) {
